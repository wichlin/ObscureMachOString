...
//----------------------------
- (NSArray<NSValue *> *)getCStringNodeRangeArray {
    ...
    for (SectionVector::const_iterator sectIter = ++sections.begin(); sectIter != sections.end(); ++sectIter)
    {
      ...
      switch (section->flags & SECTION_TYPE)
      {
        case S_CSTRING_LITERALS:
              NSRange range = {section->offset + imageOffset, section->size};
...
- (NSArray<NSValue *> *)getCString64NodeRangeArray {
    NSMutableArray<NSValue *> *array = [[NSMutableArray alloc] init];
    for (Section64Vector::const_iterator sectIter = ++sections_64.begin(); sectIter != sections_64.end(); ++sectIter)
    {
      ...
      switch (section_64->flags & SECTION_TYPE)
        {
          case S_CSTRING_LITERALS:
              NSRange range = {section_64->offset + imageOffset, section_64->size};
              [array addObject:[NSValue valueWithRange:range]];
              break;
      }
    }
